...
import {
  ...
  Settings, Book, ShieldCheck, Home, Loader2, Lock
} from 'lucide-react';
...
type Tab = 'agreements' | 'templates' | 'compliance' | 'ip' | 'tasks';
...
function Legal() {
  ...
  const fetchRelations = () => {
    ...
    fetchWithAuth(`${API}/core/committees`, () => {});
    fetchWithAuth(`${API}/tech/projects`, () => {});
    fetchWithAuth(`${API}/core/employees`, () => {});
    fetchWithAuth(`${API}/tech/environments`, () => {});
  };
    ...
    <div className="min-h-screen flex flex-col bg-background font-sans text-textPrimary animate-in fade-in duration-700">
      <header className="glass-panel sticky top-0 z-50 px-8 py-4 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-black tracking-tighter leading-none">GAnova<span className="text-amber-400">LEGAL</span></h1>
            <span className="text-[10px] uppercase tracking-[0.2em] text-textSecondary font-black leading-none">General Counsel Office</span>
          </div>
          <button onClick={() => window.location.href = '/'} className="ml-2 p-2 text-textSecondary hover:text-amber-400 hover:bg-amber-400/10 rounded-xl transition-all">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setShowProfile(true)} className="flex items-center gap-3 pl-2 pr-4 py-1.5 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 transition-all group">
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 flex items-center justify-center font-bold text-xs shadow-lg shadow-amber-500/20">
        ...
        <div className="flex gap-2 max-w-7xl mx-auto">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              <t.icon className={`w-3.5 h-3.5 ${tab === t.id ? 'text-amber-400' : 'text-textSecondary'}`} />
              {t.label}
            </button>
          ))}
        </div>
      ...
      <main className="flex-1 p-8 max-w-7xl mx-auto w-full space-y-8 animate-in slide-in-from-bottom-2 duration-500">
        {tab !== 'tasks' && (
          <GanovaGrid
            ...
            columns={
              tab === 'agreements' ? [
                { key: 'agreementName', label: 'Agreement', type: 'avatar' },
                { key: 'status', label: 'Status', type: 'status' },
                { key: 'expiryDate', label: 'Expiry', type: 'date' },
              ] : tab === 'templates' ? [
                { key: 'documentName', label: 'Template' },
                { key: 'versionNumber', label: 'Version', type: 'badge' },
              ] : tab === 'compliance' ? [
                { key: 'obligationName', label: 'Obligation' },
                { key: 'status', label: 'Status', type: 'status' },
                { key: 'dueDate', label: 'Due Date', type: 'date' },
              ] : [
                { key: 'assetName', label: 'Asset', type: 'avatar' },
            ...
            onDelete={async (r) => {
              if (!confirm(`Irreversible deletion of legal record. Confirm?`)) return;
              await fetch(`${API}/legal/${tab}/${r.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token()}` } });
              fetchData();
            }}
            canAdd={p.canEdit}
            canEdit={p.canEdit}
      ...
      {showEntityForm && (
        <EntityForm
          ...
          fields={
            tab === 'agreements' ? [
              { key: 'agreementName', label: 'Title', type: 'text', required: true },
              { key: 'status', label: 'Status', type: 'select', options: [{ value: 'active', label: 'Active' }, { value: 'pending', label: 'Pending' }, { value: 'expired', label: 'Expired' }], required: true },
            ] : tab === 'templates' ? [
              { key: 'documentName', label: 'Name', type: 'text', required: true },
              { key: 'versionNumber', label: 'Version', type: 'text' },
            ] : tab === 'compliance' ? [
              { key: 'obligationName', label: 'Obligation', type: 'text', required: true },
              { key: 'status', label: 'Status', type: 'select', options: [{ value: 'active', label: 'Active' }, { value: 'pending', label: 'Pending' }], required: true },
            ] : [
