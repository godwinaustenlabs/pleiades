 ...
 * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      <div className="bg-background rounded-3xl w-full max-w-2xl max-h-[92vh] overflow-hidden shadow-2xl border border-white/20 animate-in fade-in zoom-in duration-200 flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/10 bg-white/5 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button onClick={handlePrint} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-surface text-xs font-bold hover:opacity-90 transition-all">
 ...
 record.disbursementStatus === 'paid' ? 'bg-success/10 text-success border-success/20' :
 record.disbursementStatus === 'processed' ? 'bg-info/10 text-info border-info/20' :
 'bg-warning/10 text-warning border-warning/20'
            ...
            <div className="rounded-2xl overflow-hidden border border-success/20">
              <div className="bg-success/10 px-4 py-2.5 flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-success uppercase tracking-widest">Earnings</span>
              </div>
                ...
                <tbody className="divide-y divide-white/5">
                  {displayEarnings.map((e, i) => (
                    <tr key={i} className="hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-2.5 text-right font-mono font-bold text-success">{fmt(e.amount)}</td>
                    </tr>
                  ))}
                  <tr className="bg-success/5 border-t border-success/20">
                    ...
                    <td className="px-4 py-3 text-right font-mono font-black text-success text-base">{fmt(record.grossSalary)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            {displayDeductions.length > 0 && (
              <div className="rounded-2xl overflow-hidden border border-danger/20">
                <div className="bg-danger/10 px-4 py-2.5 flex items-center gap-2">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {displayDeductions.map((d, i) => (
                      <tr key={i} className="hover:bg-white/[0.02]">
                        ...
                        <td className="px-4 py-2.5 text-right font-mono font-bold text-danger">− {fmt(d.amount)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-r from-primary/10 to-accent/5 border border-primary/30 flex items-center justify-between">
              ...
              <div className="text-right space-y-1">
                ...
                <div className="font-mono font-bold text-sm text-danger">
            ...
            <div className="flex items-center justify-between pt-4 border-t border-white/5 text-[10px] text-textSecondary">
              <p>Generated by Pleiades HR · {new Date().toLocaleDateString()}</p>
